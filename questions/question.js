...
const main = async () => {
  ...
  Promise.all(
    dataServices.map(async (service, i) => {
      if(service.spec_questions.length > 0){
        service.spec_questions.map(async (questionId, index) => {
          const item = await findQuestionMongo(service.spec_questions[index].$oid);
          if(item !== undefined){
    ...
    fs.writeFile('pg-questions.json',questionsStringify,'utf8', function(err) {
      if (err) throw err;
      console.log('complete');
      }
  );
  fs.writeFile('pg-questionsTexts.json', questionsTextStringify,'utf8', function(err) {
      if (err) throw err;
      console.log('complete');
      }
  );
